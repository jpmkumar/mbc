...
\textbf{Hardware.} PyTorch + PennyLane \texttt{default.qubit} simulator; GPU when available.

\section{Results}
Table~\ref{tab:main} summarizes core experiments. Leave-one-modality-out (E4) evaluates cross-modality generalization by training on two modalities and testing on the held-out third. Ablation studies (E6) quantify modality token and Transformer contributions. Qubit sweep (E7) replicates pilot study sensitivity analysis for 4, 6, and 8 qubits.

\begin{table}[!t]
\caption{Core experiment results (synthetic validation run; replace with real-dataset numbers before submission).}
\label{tab:main}
\centering
\begin{tabular}{lccc}
\toprule
...
\midrule
Classical Unified (E2) & --- & 0.471 & --- \\
Hybrid Full (E3) & --- & 0.471 & --- \\
LOMO Mammography & --- & 0.471 & --- \\
LOMO Ultrasound & --- & --- & --- \\
LOMO Thermography & --- & --- & --- \\
\bottomrule
\end{tabular}
...
\textbf{Limitations.} Simulation-only quantum evaluation; synthetic or public datasets may not capture full clinical diversity; two-stage training adds complexity.
